...
const PAGE1_STEPS = [
  { icon: '📍', title: '여행 지역 선택', body: '시/도 → 시/군/구 순서로 다녀올 지역을 골라요. 둘 다 가나다순으로 나와서 찾기 쉬워요.' },
  { icon: '📅', title: '여행 날짜 선택', body: '바람따라는 당일치기만 지원해요. 하루 날짜만 고르면 되고, 지난날은 고를 수 없어요. 같은 날 진행 중인 여행이 있으면 이어하기나 새로 만들기를 고를 수 있어요.' },
  { icon: '🧑‍🤝‍🧑', title: '누구와 함께하나요?', body: '1인·2인·3인·4인 가족·대가족 중 고르고, 성인 연령대를 골라요. 자녀가 있으면 나이를 넣고, 반려동물·유모차·무장애가 필요하면 함께 체크해요.' },
];
...
const PAGE2_STEPS = [
  {
    icon: '🧭',
    title: '홈 · 일정 · 지도 · 검색 · 알림 · 프로필',
    body: '일정 화면은 아래 탭 6개로 오가요. 「홈」을 누르면 새 여행을 시작하는 화면으로 돌아가고, 「일정」에서 오늘 일정을 보고, 지도에서 동선을, 검색에서 장소를 더 담고, 알림에서 온 메시지와 오늘 날씨·중기 예보를, 프로필에서 글씨·알림·마무리를 다뤄요.',
  },
  {
    icon: '📋',
    title: '일정 카드 다루기',
    body: '장소 이름을 누르면 상세가 펼쳐져요. 왼쪽 시각을 눌러 방문 시간을 바꾸고, 「⏱ 시간순 정렬」로 목록 순서를 맞출 수 있어요. 카드 왼쪽 동그라미를 누르면 다녀온 곳으로 표시돼요(다시 누르면 되돌려요) — 다녀온 곳은 목록 아래 「지난 일정」으로 접혀 모이니, 남은 일정에만 집중할 수 있어요. 방문 예정 시각이 지났는데 아직 표시 안 한 곳이 있으면 「OO 다녀오셨나요?」라고 먼저 물어봐요. 펼친 카드에서는 ✓ 다녀옴 · ✏️ 수정 · 🎧 도슨트 · 🗑️ 삭제를 쓸 수 있어요.',
  },
  {
    icon: '🏷️',
    title: '장소 더 담기',
    body: '검색 탭에서 「근처 기준」으로 오늘 일정 중 한 곳을 고른 뒤, 맛집·카페 / 실내 / 역사·전통 / 자연·액티비티 / 쇼핑·체험 태그를 고르고 「추천받기」를 눌러요. 카드에는 지금 이 순간 영업중인지 휴무·마감인지, 예약이 필요한 곳인지도 함께 떠요. 맛집이면 1인 식사 참고 금액을, 그 외에는 무료 장소만 보기를 켤 수 있어요. 지도에서는 옮긴 뒤 「이 지역 재검색」으로 주변을 담아요. 어디서 담든 담자마자 「일정」 탭으로 이동해서 바로 확인할 수 있어요. 방문일이 휴무이거나 마감에 가까우면 「그래도 담기 / 취소」로 물어보고, 다른 일정과 시간이 겹치면 담지 않아요.',
  },
  {
    icon: '🌀',
    title: '변수에 바로 대응하기',
    body: '홈 위쪽 바람개비와 「지금 변수에 대응하기」 세 장(비·폭염 / 혼잡 / 동선)이 여행 당일 이미 담은 일정을 90초마다 다시 봐요. 일정 카드의 🟡 주의·🔴 변경 필요 배지에는 혼잡·폭염·우천처럼 실제 원인이 그대로 표시돼서 왜 그런지 한눈에 알 수 있어요. 반짝이면 실내·한산한 곳·짧은 동선으로 바로 바꿀 수 있어요. 휴무·마감·이동시간 부족도 바람개비가 잡아 주고, 「동선 다시」로 순서를 다시 짤 수 있어요. 이렇게 바꾼 일정은 「🕓 변경 이력」에 쌓여서, 맨 처음 짠 원본과 비교하거나 예전 시점으로 되돌릴 수 있어요(되돌리기도 이력에 남아요). 바뀐 장소 카드엔 「🟡 변경됨」 표시가 붙어요.',
  },
  {
    icon: '✅',
    title: '알림 · 마무리 · 공유',
    body: '프로필에서 알림을 켜 두면 여행 당일에만 와요. 순풍일 때는 첫 일정 30분 전과 여행 마무리를, 주황·빨강(비·폭염·혼잡·동선·휴무)은 감지되는 즉시 휴대폰으로 알려드리고, 눌렀을 때 바로 해당 화면·장소로 이동해요. 글씨 크기도 여기서 키울 수 있어요. 「여행 마무리」는 좋았음·보통·별로만 누르면 되고, 별로인 장소는 다음 추천에서 빼 드려요. 좋았음이면 같은 지역 추천 코스로 쓰일 수 있어요. 홈의 「공유」로 링크를 보내고, 메뉴의 「내 여행 관리」에서 지난 일정을 볼 수 있어요.',
  },
];
