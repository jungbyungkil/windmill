...
public class TripRecordService {
    ...
    /**
     * 첫 화면 인기 여행 기록 피드 - 선택한 지역의 당일치기만.
     * 지역 코드가 없거나 해당 지역 기록이 없으면 빈 목록(프론트는 섹션 자체를 숨김).
     */
    @Transactional(readOnly = true)
    public List<TripStoryFeedResponse> findPopularStories(String signguFullCode) {
        ...
        return tripRecordRepository.findDayTripStoriesByRegion(signguFullCode, PageRequest.of(0, 5))
                .stream()
